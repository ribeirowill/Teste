...
public class TipoSistemasImpl implements TipoSistemasQueries {
	...
	public Page<TipoSistema> filtrar(TipoSistemaFilter filtro, Pageable pageable) {
		Criteria criteria = manager.unwrap(Session.class).createCriteria(TipoSistema.class);
	...
	private Long total(TipoSistemaFilter filtro) {
		Criteria criteria = manager.unwrap(Session.class).createCriteria(TipoSistema.class);
		adicionarFiltro(filtro, criteria);
	...
	private void adicionarFiltro(TipoSistemaFilter filtro, Criteria criteria) {
		if (filtro != null) {
